Ignite упрощает использование Firecracker за счет переноса опыта разработчиков из (containers). Используя Ignite, вы выбираете OCI-compliant image (Docker image), который хотите запустить как виртуальную машину, а затем просто выполняете ignite run вместо docker run. Нет необходимости использовать специальные инструменты для виртуальной машины для создания образов .vdi, .vmdk или .qcow2, просто выполните сборку (OCI-compliant image (docker build) из любого необходимого базового образа (например, ubuntu: 18.04 из Docker Hub) и добавьте желаемое содержимое.

Когда вы запускаете свой (OCI image)с помощью ignite run, Firecracker загрузит новую виртуальную машину примерно за 125 миллисекунд (!) Для вас, используя ядро Linux 4.19 по умолчанию. Если вы хотите использовать какое-то другое ядро, просто укажите --kernel-image flag, указывающий на другой образ (OCI image), содержащий ядро в /boot/vmlinux, и, возможно, ваши предпочтительные модули. Затем ядро выполняет /sbin/init на виртуальной машине VM,, и все запускается. После этого Ignite подключает виртуальные машины к любой сети CNI, интегрируясь, например, с Weave Net.
